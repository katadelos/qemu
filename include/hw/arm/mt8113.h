...
typedef struct MT8113USBState {
    ...
    qemu_irq irq;
    uint8_t ep0_fifo[64];
    uint8_t control_in[512];
    uint8_t control_out[64];
    qemu_irq vbus;
    unsigned ep0_fifo_length;
    unsigned ep0_fifo_offset;
    unsigned control_in_length;
    unsigned control_out_length;
    unsigned config_phase;
    uint8_t config_value;
    uint8_t control_interface;
    uint8_t data_interface;
    bool rndis;
    bool control_tx_pending;
    bool setup_pending;
    bool configured;
    bool processing_tx;
} MT8113USBState;
...
struct MT8113State {
    ...
    uint64_t wifi_tx_packets;
    uint64_t wifi_rx_packets;
    uint8_t wifi_wlan_index;
    uint8_t wifi_bss_index;
    uint8_t wifi_scan_pending;
    uint8_t wifi_scan_sequence;
    bool wifi_sched_enabled;
    bool wifi_sched_matches;
    bool wifi_sched_stop_after;
    bool wifi_sched_event_pending;
    uint8_t wifi_sched_sequence;
    uint32_t wifi_sched_interval;
    int64_t wifi_sched_next;
    uint16_t wifi_rx_sequence;
    uint16_t wifi_management_length;
    uint8_t wifi_management[256];
